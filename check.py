...
def establish_tcp_connection(proxy_url):
    ...
    if proxy_url.scheme == "https":
        ...
        # ← disable hostname check (optional but recommended here)
        context.check_hostname = False
        # ← THIS disables certificate verification → bypasses self-signed error
        context.verify_mode = ssl.CERT_NONE
        retSock = context.wrap_socket(
            sock,
            # still send SNI (important for many servers)
